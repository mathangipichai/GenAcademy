...
I implemented the following prompt engineering and control flow updates to address the identified failure modes:
...
* **Details of the Fix**:
  - I added a new node `input_guardrail` at the start of the LangGraph state machine.
...
* **Details of the Fix**:
  - I updated the system prompt of `find_competitors_node` in `graph.py`.
...
* **Details of the Fix**:
  - I modified the system prompt of `compile_draft_report_node` in `graph.py`.
